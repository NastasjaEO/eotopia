# -*- coding: utf-8 -*-
"""
The module implements the core graph data structure.
It is used, for example, to model dependencies among tasks in the
ooiworkflow module.
...
class DirectedGraph:
    ...
    Constructs a new graph from an adjacency list. 
    If adjacency_dict is None, an empty graph is constructed.
    
    :param adjacency_dict: A dictionary mapping vertices to lists neighbors
    """

    def __init__(self, adjacency_dict=None):
        self.adj_dict = collections.defaultdict(list, adjacency_dict)\
            if adjacency_dict else collections.defaultdict(list)
        self.indegrees = DirectedGraph._get_indegrees(self.adj_dict)
        self._vertices =\
            set(self.adj_dict.keys()) | {v for neighs in self.adj_dict.values()\
                                         for v in neighs}
    ...
    def get_indegrees(self):
        ...
        **Note** that if ``u`` is not in graph then ``indegrees[u]=0`` 
        due to ``defaultdict`` behavior!
        """
        return self.indegrees

    def get_indegree(self, vertex):
        """Returns the in-degree of the vertex.
        The in-degree is the number of vertices ``vertex'`` such that 
        ``vertex' -> vertex`` is an edge of the graph.
        :param vertex: Vertex
        """
    ...
    def get_outdegree(self, vertex):
        ...
        The out-degree is the number of vertices ``vertex'`` such that 
        ``vertex -> vertex'`` is an edge of the graph.
        :param vertex: Vertex
        """
    ...
    def add_edge(self, u_vertex, v_vertex):
        """
        Adds the edge ``u_vertex -> v_vertex`` to the graph if the edge is 
        not already present.
    ...
    def del_edge(self, u_vertex, v_vertex):
        """
        Removes the edge ``u_vertex -> v_vertex`` from the graph if the 
        edge is present.
    ...
    def _is_cyclic(dag):
        ...
        The algorithm is naive, running in O(V^2) time, and not intended 
        for serious use! For production purposes on
