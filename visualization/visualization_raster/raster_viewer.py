# -*- coding: utf-8 -*-
"""
Created on Mon May 24 07:31:29 2021
...
from ipywidgets import (interactive_output, IntSlider, Layout, Checkbox, 
                        Button, HBox, Label, VBox)
...
class RasterViewer(object):
    ...
    """
    def __init__(self, path, cmap='jet', band_indices=None, band_names=None, 
                 pmin=2, pmax=98, zmin=None, zmax=None, ts_convert=None, title=None, 
                 datalabel='data', spectrumlabel='time', 
                 format='ENVI', fontsize=8, custom=None):
...
#        self.ylab = ylab.lower() if ylab is not None else 'latitude'
        ...
        if band_indices is not None:
            if len(list(set(band_indices))) != self.bands:
                raise RuntimeError('length mismatch of unique provided band indices ({0}) '
                                   'and image bands ({1})'\
                                       .format(len(band_indices), self.bands))
            else:
        ...
        # a simple checkbox to enable stacking of vertical profiles into one plot
    ...
    def __onslide(self, h):
        ...
        img = self.__read_band(self.band + 1)
    ...
    def __init_vertical_plot(self):
        ...
        self.vline = self.ax2.axvline(self.timestamps[self.slider.value], 
                                      color='black')

    def __onclick(self, event):
        """
        respond to mouse clicks in the plot.
        This function responds to clicks on the first (horizontal slice) plot 
        and updates the vertical profile and slice plots
        Parameters
        ----------
 ...
 #                                              values=timeseries)
  #                      func(**args)
    ...
    def csv(self, outname=None):
        ...
        outname: str
            the name of the file to write; if left at the default `None`, 
            a graphical file selection dialog is opened
    ...
    def get_current_profile(self):
        """
        Returns list of the values of the most recently plotted time series
        """
        profiles = self.ax2.get_lines()[1:]
        if len(profiles) == 0:
            return []
