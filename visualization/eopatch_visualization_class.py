...
class EOPatchVisualization:
    ...
    def plot_data(self, feature_name):
        ...
        data_da = feature_array_to_dataframe(self.eopatch, 
                                             (FeatureType.DATA, 
                                              feature_name), crs=crs)
        timestamps = self.eopatch.timestamp
        crs = self.eopatch.bbox.crs
        data_rgb = self.eopatch_dataframe_to_rasterband(data_da, 
                                                        feature_name, crs)
        rgb_dict = {timestamp_: self.plot_one_band(data_rgb, timestamp_)\
    ...
    def plot_vector_timeless(self, feature_name):
        ...
        if crs is CRS.WGS84:
            crs = CRS.POP_WEB
            data_gpd = data_gpd.to_crs(crs.pyproj_crs())
        return gv.Polygons(data_gpd, crs=ccrs.epsg(crs.epsg), 
                           vdims=self.vdims)

    def eopatch_dataframe_to_rasterband(self, eopatch_da, feature_name, crs):
        """ 
        Creates new xarray DataArray (from old one) to plot one band image 
        with hv.Holomap
