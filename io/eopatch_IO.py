...
class ExportToTiff(BaseLocalIo):
    ...
    When exporting multiple times OR bands, the Geo-Tiff `band` counts are in 
    the expected order.
    However, when exporting multiple times AND bands, the order obeys the 
    following pattern:
    ...
    def __init__(self, feature, folder=None, *, band_indices=None, date_indices=None, 
                 crs=None, fail_on_missing=True, compress=None, **kwargs):
        """
        :param feature: Feature which will be exported
        :type feature: (FeatureType, str)
        :param folder: A directory containing image files or a path of an image file.
            If the file extension of the image file is not provided, it will default 
            to ".tif".
            If a "*" wildcard or a datetime.strftime substring (e.g. "%Y%m%dT%H%M%S")
            is provided in the image file, an EOPatch feature will be split over 
            multiple GeoTiffs each corresponding to a timestamp,
            and the stringified datetime will be appended to the image file name.
        :type folder: str
        :param band_indices: Bands to be added to tiff image. Bands are represented 
            by their 0-based index as tuple in the inclusive interval form 
            `(start_band, end_band)` or as list in the form
            `[band_1, band_2,...,band_n]`.
        :type band_indices: tuple or list or None
        :param date_indices: Dates to be added to tiff image. 
            Dates are represented by their 0-based index as tuple in the 
            inclusive interval form `(start_date, end_date)` or a list in 
            the form `[date_1, date_2,...,date_n]`.
        ...
        :param fail_on_missing: should the pipeline fail if a feature is missing 
            or just log warning and return
        :type fail_on_missing: bool
        :param compress: the type of compression that rasterio should apply 
            to exported image.
    ...
    def _prepare_image_array(self, eopatch, feature):
        """ 
        Collects a feature from EOPatch and prepares the array of an image 
        which will be rasterized. The resulting array has shape 
        (channels, height, width) and is of correct dtype.
        """
    ...
    def _get_bands_subset(self, array):
        ...
        if isinstance(self.band_indices, list):
            if [band for band in self.band_indices if not isinstance(band, int)]:
                raise ValueError('Invalid format in {} list, expected integers'\
                                 .format(self.band_indices))
            return array[..., self.band_indices]
        if isinstance(self.band_indices, tuple):
            if tuple(map(type, self.band_indices)) != (int, int):
                raise ValueError('Invalid format in {} tuple, expected integers'\
                                 .format(self.band_indices))
            return array[..., self.band_indices[0]: self.band_indices[1] + 1]
        raise ValueError('Invalid format in {}, expected tuple or list'\
                         .format(self.band_indices))
    ...
    def _get_dates_subset(self, array, dates):
        ...
        if isinstance(self.date_indices, tuple):
            ...
            if tuple(map(type, self.date_indices)) == (int, int):
                ...
                raise ValueError('Invalid format in {} tuple, expected ints,\
                                 strings, or datetimes'.format(self.date_indices))
            return array[np.nonzero(np.where((dates >= start_date)\
                                             & (dates <= end_date), dates, 0))[0]]
    ...
    def _set_export_dtype(self, data_array, feature):
        ...
        if image_dtype == np.int64:
            ...
            warnings.warn('Data from feature {} cannot be exported to tiff with\
                          dtype numpy.int64. Will export as numpy.int32 instead'\
                              .format(feature))
    ...
    def _export_tiff(self, image_array, filesystem, path, channel_count,
                     dst_crs, dst_height, dst_transform, dst_width, 
    ...
    def execute(self, eopatch, *, filename=None):
        ...
        :param filename: A filename of tiff file or None if entire path has 
            already been specified in `folder` parameter of task initialization.
            If the file extension of the image file is not provided, it will default to ".tif".
            If a "*" wildcard or a datetime.strftime substring (e.g. "%Y%m%dT%H%M%S")
            is provided in the image file, an EOPatch feature will be split over 
            multiple GeoTiffs each corresponding to a timestamp, and the 
            stringified datetime will be appended to the image file name.
        ...
        src_transform =\
            rasterio.transform.from_bounds(*eopatch.bbox, width=width, height=height)
        ...
        with filesystem:
            if len(filename_paths) > 1:
                ...
                for timestamp_index, path in enumerate(filename_paths):
                    time_slice_array =\
                        image_array[timestamp_index * channel_count:
                                    (timestamp_index + 1) * channel_count, ...]

                    self._export_tiff(time_slice_array, filesystem, path, 
                                      channel_count, dst_crs, 
                                      dst_height, dst_transform, dst_width, 
                                      src_crs, src_transform)
            else:
                self._export_tiff(image_array, filesystem, filename_paths[0], 
                                  channel_count, dst_crs, 
                                  dst_height, dst_transform, dst_width, 
                                  src_crs, src_transform)
        return eopatch
