...
class SentinelHubInputTask(SentinelHubInputBase):
    """ 
    Process API input task that loads 16bit integer data and converts it to a 
    32bit float feature.
    """
    ...
    CUSTOM_BAND_TYPE = ProcApiType("custom", 'REFLECTANCE', 'FLOAT32', 
                                   np.float32, FeatureType.DATA)

    def __init__(self, data_collection=None, size=None, resolution=None, 
                 bands_feature=None, bands=None, additional_data=None, 
                 evalscript=None, maxcc=None, time_difference=None, cache_folder=None,
        ...
        :param bands: An array of band names. If not specified it will 
            download all bands specified for a given data collection.
        :type bands: list[str]
        :param additional_data: A list of additional data to be downloaded, 
            such as SCL, SNW, dataMask, etc.
        :type additional_data: list[tuple(sentinelhub.FeatureType, str)]
        :param evalscript: An optional parameter to override an evascript that 
            is generated by default
        :type evalscript: str or None
        :param maxcc: Maximum cloud coverage.
        :type maxcc: float
        :param time_difference: Minimum allowed time difference, 
            used when filtering dates, None by default.
        :type time_difference: datetime.timedelta
        :param cache_folder: Path to cache_folder. If set to None (default) 
            requests will not be cached.
        :type cache_folder: str
        ...
        :param single_scene: If true, the service will compute a single image 
            for the given time interval using mosaicking.
        ...
        self.bands_feature = next(self._parse_features(bands_feature, 
        ...
        if additional_data is not None:
            additional_data = list(self._parse_features(additional_data, 
                                                        new_names=True)())
            self._add_request_bands(self.requested_bands, (band for\
                                                           ftype, band, new_name\
                                                               in additional_data))
    ...
    def _add_request_bands(request_dict, added_bands):
        ...
        for band in added_bands:
            found = next(((btype, band) for btype, bands\
                          in predefined_types if band in bands), None)
    ...
    def generate_evalscript(self):
        ...
        outputs = [
            "{{ id:{id}, bands:{num_bands}, sampleType: SampleType.{sample_type} }}".format(
                id='\"{}\"'.format(btype.id), num_bands=len(bands), 
                sample_type=btype.sample_type
            )
            for btype, bands in self.requested_bands.items()
        ]

        samples = [
            (btype.id, '[{samples}]'.format(samples=', '.join("sample.{}"\
                                                              .format(band)\
                                                                  for band in bands)))
            for btype, bands in self.requested_bands.items()
        ]

        # return value of the evaluatePixel has to be a list if we're returning 
        # just one output, and a dict otherwise
        # an issue has been reported to the service team and this might get fixed
        if len(samples) == 1:
            _, sample_bands = samples[0]
            samples = sample_bands
        else:
            samples = ', '.join('{band_id}: {bands}'\
                                .format(band_id=band_id, bands=bands)\
                                    for band_id, bands in samples)
            samples = '{{{samples}}};'.format(samples=samples)

        bands = ["\"{}\"".format(band) for bands in self.requested_bands.values()\
                 for band in bands]

        units = (unit.unit for btype, bands in self.requested_bands.items()\
                 for unit, band in zip(repeat(btype), bands))
        units = ["\"{}\"".format(unit) for unit in units]

        evalscript = evalscript.format(
            bands=', '.join(bands), units=', '.join(units), 
            outputs=', '.join(outputs), samples=samples
        )
    ...
    def _create_sh_request(self, date_from, date_to, bbox, size_x, size_y):
        ...
        Create an instance of SentinelHubRequest
        """
        responses = [SentinelHubRequest.output_response(btype.id, MimeType.TIFF)\
    ...
    def _extract_additional_features(self, eopatch, images, shape):
        ...
        """
        feature = {band: (ftype, new_name)\
                   for ftype, band, new_name in self.additional_data}
        for btype, tifs, bands in self._iter_tifs(images, 
                                                  ['bool_mask', 'mask', 
                                                   'uint8_data', 'other']):
            for band in bands:
                eopatch[feature[band]] = self._extract_array(tifs, bands.index(band), 
                                                             shape, btype.np_dtype)
    ...
    def _extract_bands_feature(self, eopatch, images, shape):
        ...
        itr = [(btype, images, bands, bands.index(band))\
               for btype, images, bands in tifs for band in bands]
        bands = [self._extract_array(images, idx, shape, self.bands_dtype, norms)\
                 for btype, images, band, idx in itr]
    ...
    def _iter_tifs(self, tars, band_types):
        ...
        return ((btype, [tar[btype.id + '.tif']\
                         for tar in tars], self.requested_bands[btype])\
                for btype in rtypes)

    @staticmethod
    def _extract_array(tifs, idx, shape, dtype, norms=None):
        """ 
        Extract a numpy array from the received tifs and normalize it if 
        normalization factors are provided
        """
