...
class EOPatch:
    ...
    def find_index_of_feature(self, feature_name="DATA", item_name=None, 
                           searchby="feat"):
        """Returns a list indeces for a specific feature or item name.

        :return: list of indeces
        :rtype: list(int)
        
        Examples:
        find_index_of_feature(eopatch, feature_name="MASK")
        find_index_of_feature(eopatch, item_name="Mybeautifuldata")
        """

        feats = self.get_feature_list()
        if not item_name or searchby == "feat":
            index = []
            for i in range(len(feats)):
                if not isinstance(feats[i], tuple):
                    continue
                feat = feats[i][0].name
                if feat == feature_name:
                    index.append(i)
        
        elif item_name and searchby == "item":
            index = []
            for i in range(len(feats)):
                if not isinstance(feats[i], tuple):
                    continue
