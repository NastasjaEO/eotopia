# -*- coding: utf-8 -*-
"""
The module handles execution and monitoring of workflows. 
It enables executing a workflow multiple times and in parallel. It monitors 
execution times and handles any error that might occur in the process. 
At the end it generates a report which contains summary of the workflow and 
...
from OOI_workflow_classes import (OOIWorkflow)
...
class OOIExecutor:
    ...
    In the process it monitors execution and handles errors. 
    It can also save logs and create a html report about each execution.
    """
    ...
    def __init__(self, workflow, execution_args, *, save_logs=False, 
                 logs_folder='.', logs_filter=None, execution_names=None):
        """
        :param workflow: A prepared instance of OOIWorkflow class
        :type workflow: OOIWorkflow
        :param execution_args: A list of dictionaries where each dictionary 
            represents execution inputs for the workflow. 
            `OOIExecutor` will execute the workflow for each of the given 
            dictionaries in the list. The content of such dictionary will be 
            used as `input_args` parameter in `OOIWorkflow.execution` method.
            Check `OOIWorkflow.execution` for definition of a dictionary structure.
        :type execution_args: list(dict(OOITask: dict(str: object) or tuple(object)))
        :param save_logs: Flag used to specify if execution log files should 
            be saved locally on disk
        :type save_logs: bool
        :param logs_folder: A folder where logs and execution report should be saved
        :type logs_folder: str
        :param logs_filter: An instance of a custom filter object that 
            will filter certain logs from being written into logs. 
            It works only if save_logs parameter is set to True.
        :type logs_filter: logging.Filter or None
        :param execution_names: A list of execution names, which will be shown 
            in execution report
    ...
    def _parse_execution_args(execution_args):
        """ 
        Parses execution arguments provided by user and raises an error 
        if something is wrong
        """
        if not isinstance(execution_args, (list, tuple)):
            raise ValueError("Parameter 'execution_args' should be a list")
        return [OOIWorkflow.parse_input_args(input_args)\
    ...
    def _parse_execution_names(execution_names, execution_args):
        ...
        if not isinstance(execution_names, (list, tuple)) or\
            len(execution_names) != len(execution_args):
            raise ValueError("Parameter 'execution_names' has to be a list of\
                             the same size as the list of execution arguments")
    ...
    def run(self, workers=1, multiprocess=True, return_results=False):
        ...
        :param workers: Maximum number of workflows which will be executed in 
            parallel. Default value is `1` which will execute workflows consecutively. 
            If set to `None` the number of workers will be the number of processors
            of the system.
        :type workers: int or None
        :param multiprocess: If `True` it will use 
            `concurrent.futures.ProcessPoolExecutor` which will distribute
            workflow executions among multiple processors. If `False` it will use
            `concurrent.futures.ThreadPoolExecutor` which will distribute workflow 
            among multiple threads.
            However even when `multiprocess=False`, tasks from workflow could 
            still be using multiple processors.
            This parameter is used especially because certain task cannot run with
            `concurrent.futures.ProcessPoolExecutor`.
            In case of `workers=1` this parameter is ignored and workflows will 
            be executed consecutively.
        :type multiprocess: bool
        :param return_results: If `True` this method will return a list of all 
            results of the execution. Note that this might exceed the 
            available memory. By default this parameter is set to `False`.
        :type: bool
        :return: If `return_results` is set to `True` it will return a list of 
            results, otherwise it will return `None`
        :rtype: None or list(eotopia.core.WorkflowResults)
        """
        ...
        processing_args = [(self.workflow, init_args, log_path, return_results, 
                            filter_logs_by_thread)
                               for init_args, log_path in\
                                   zip(self.execution_args, log_paths)]

        if workers == 1:
            processing_type = 'single process'
            self.execution_stats =\
                list(tqdm(map(self._execute_workflow, processing_args), 
            ...
            with pool_executor_class(max_workers=workers) as executor:
                self.execution_stats = list(tqdm(executor.map(self._execute_workflow, 
                                                              processing_args),
                                                 total=len(processing_args)))
        ...
        return [stats.get(self.RESULTS) for stats in self.execution_stats] if\
            return_results else None

    def _try_add_logging(self, log_path, filter_logs_by_thread):
        """ 
        Adds a handler to a logger and returns them both. 
        In case this fails it shows a warning.
        """
        if log_path:
            try:
    ...
    def _try_remove_logging(cls, log_path, logger, handler, stats):
        ...
        if log_path:
            try:
                message = 'OOIWorkflow execution {}'.\
                    format('failed' if cls.STATS_ERROR in stats else 'finished')
    ...
    def _execute_workflow(self, process_args):
        ...
        """
        workflow, input_args, log_path, return_results, filter_logs_by_thread =\
            process_args
    ...
    def _get_log_handler(self, log_path, filter_logs_by_thread):
        ...
        """
        handler = logging.FileHandler(filter_logs_by_thread)
...
#         if self.logs_filter ## TODO!
        if self.logs_filter or self.filter_logs_by_thread:
    ...
    def _get_report_folder(self):
        ...
        return os.path.join(self.logs_folder,
                            'eoexecution-report-{}'.\
                                format(self.start_time.strftime("%Y_%m_%d-%H_%M_%S")))
    ...
    def get_successful_executions(self):
        """ 
        Returns a list of IDs of successful executions. 
        The IDs are integers from interval
    ...
    def get_failed_executions(self):
        """ 
        Returns a list of IDs of failed executions. 
        The IDs are integers from interval
        `[0, len(execution_args) - 1]`, sorted in increasing order.
...
def execute_with_mp_lock(execution_function, *args, **kwargs):
    """ 
    A helper utility function that executes a given function with multiprocessing 
    lock if the process is being executed in a multi-processing mode
