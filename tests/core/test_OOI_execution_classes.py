...
from OOI_workflow_classes import (OOIWorkflow, Dependency, WorkflowResults,
                                  LinearWorkflow)
from OOI_execution_classes import (OOIExecutor, execute_with_mp_lock)


logging.basicConfig(level=logging.DEBUG)

class ExampleTask(OOITask):
...
class FooTask(OOITask):

    @staticmethod
    def execute(*_, **__):
        return 42

class KeyboardExceptionTask(OOITask):
...
class TestOOIExecutor(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        cls.task = ExampleTask()
        cls.final_task = FooTask()
        cls.workflow = OOIWorkflow([(cls.task, []),
                                   Dependency(task=cls.final_task, 
                                              inputs=[cls.task, cls.task])])
    ...
    def test_execution_logs_single_process(self):
        self._run_and_test_execution(workers=1, multiprocess=True, 
                                     filter_logs=False)
        self._run_and_test_execution(workers=1, multiprocess=False, 
                                     filter_logs=True)

    def test_execution_logs_multiprocess(self):
        self._run_and_test_execution(workers=5, multiprocess=True, 
                                     filter_logs=False)
        self._run_and_test_execution(workers=3, multiprocess=True, 
                                     filter_logs=True)

    def test_execution_logs_multithread(self):
        self._run_and_test_execution(workers=3, multiprocess=False, 
                                     filter_logs=False)
        self._run_and_test_execution(workers=2, multiprocess=False, 
                                     filter_logs=True)

    def _run_and_test_execution(self, workers, multiprocess, filter_logs):
        for execution_names in [None, [4, 'x', 'y', 'z']]:
            with tempfile.TemporaryDirectory() as tmp_dir_name:
                executor = OOIExecutor(self.workflow, self.execution_args, 
                                       save_logs=True,
                                       logs_folder=tmp_dir_name,
                                       logs_filter=CustomLogFilter() if\
                                           filter_logs else None,
                ...
                if execution_names:
                    for name, log_filename in zip(execution_names, log_filenames):
                        self.assertTrue(log_filename == 'ooiexecution-{}.log'.\
                                        format(name))
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            for stats in executor.execution_stats:
                for time_stat in ['start_time', 'end_time']:
                    self.assertTrue(time_stat in stats and\
                                    isinstance(stats[time_stat], 
                                               datetime.datetime))
        ...
        for multiprocess in [True, False]:
            with tempfile.TemporaryDirectory() as tmp_dir_name:
                ...
                for idx, stats in enumerate(executor.execution_stats):
                    if idx != 3:
                        self.assertFalse('error' in stats, 'Workflow {} should\
                                         be executed without errors'.format(idx))
                    else:
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(ValueError):
            OOIExecutor(self.workflow, {})

        with self.assertRaises(ValueError):
            OOIExecutor(self.workflow, self.execution_args, 
                        execution_names={1, 2, 3, 4})
        with self.assertRaises(ValueError):
            OOIExecutor(self.workflow, self.execution_args, 
                        execution_names=['a', 'b'])
    ...
    def test_keyboardInterrupt(self):
        ...
        for arg in run_args:
            self.assertRaises(KeyboardInterrupt, OOIExecutor(workflow, 
                                                execution_args).run, **arg)
...
class TestExecuteWithMultiprocessingLock(unittest.TestCase):
    ...
    def test_without_lock(self):
        with tempfile.NamedTemporaryFile() as fp:
            ...
            self.assertEqual(len(lines), 2 * self.WORKERS)
            self.assertEqual(len(set(lines[: self.WORKERS])), self.WORKERS, 
                             msg='All processes should start')
            self.assertEqual(len(set(lines[self.WORKERS:])), self.WORKERS, 
                             msg='All processes should finish')
